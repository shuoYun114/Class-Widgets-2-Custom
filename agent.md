...
- **故障根因**：
  Qt 6.8 默认编译的 Windows 平台插件 `platforms/qwindows.dll` 显式静态链接了动态缩放虚拟模块 `api-ms-win-shcore-scaling-l1-1-1.dll` 并导入 `GetDpiForMonitor`。
  Windows 10 1703 (Build 15063) 系统中仅有经典 `SHCore.dll`，不存在该 API Set 模块，导致 Windows 动态加载器（LoadLibraryEx）抛出错误码 `126 (ERROR_MOD_NOT_FOUND)`，平台插件初始化失败而弹窗崩溃。
- **防御机制与标准架构**：
  1. **资产固化**：项目仓库 `assets/shims/` 永久内置纯静态（`/MT` 编译，仅链接 `KERNEL32.dll`）的 `api-ms-win-shcore-scaling-l1-1-1.dll` 转发垫片及源码 `shim.c`、`shim.def`；
  2. **打包自动化注入**：`scripts/build_exe.py` 与 `.github/workflows/build&release.yml` 在生成可执行程序后，必须自动将该 DLL 部署至根目录、`platforms/` 以及 `PySide6/` 多层级路径；
...
5. **教室大屏文字大小与粗体调节 (Big Screen Usability)**：
   - 针对教室或大屏投影后排学生看不清课表的需求，课程名称字号支持 10px ~ 42px 宽范围调节；
   - 课程名称固定采用 DemiBold 粗体与 1.2 倍视觉比率突出显示；
   - 设置中心（General -> 小组件设置）提供独立滑块并实时全局联动。
